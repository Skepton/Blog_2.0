...
function mergeLayouts(layoutsArray){
  return jsonMerge(layoutsArray[0],layoutsArray[2]);
}
...
function loadBlock(viewpath, block, request, response, callback){
  ...
  if(childrenBlocks && childrenBlocks.length > 0){
    ...
    async.each(childrenBlocks, function(block, asyncCallback){
      loadBlock(parentViewpath, block, request, response, function(renderedBlock){
      ...
      if(!err) {
        if(block.model){
          //Start of model load
          var model = fallback("@model/"+block.model)(request, response);
        }
        var content = {};
...
function initLayout(block, request, response){
  loadBlock('', block, request, response);
}
...
module.exports = function(handles, db, request, response){
  ...
  //Render Blocks
  initLayout(mergedLayout, request, response);
}
