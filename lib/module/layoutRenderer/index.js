...
function renderBlock(block, content, request, response, callback){
...
function loadBlock(block, db, request, response, callback){
  ...
  if(childrenBlocks && childrenBlocks.length > 0){
    ...
    async.each(childrenBlocks, function(block, asyncCallback){
      loadBlock(block, db, request, response, function(renderedBlock){
      ...
      if(!err) {
        ...
        if(block.model){
          ...
          var model = fallback("@model/"+block.model)(db, request, response, function(data){
            content.data = data;
            renderBlock(block, content, request, response, function(renderedBlock){
